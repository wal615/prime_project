...
result_list_fixed_chi_un_main_0.5_inter_0.1_main_inter <- mapply(FUN = simulation_fn,
...
gene_data_args <- gene_data_args %>% split(x = ., f = seq(nrow(gene_data_args))) # generate a list from each row of a dataframe
uncorr_args <- list(p = p)
result_list_fixed_chi_un_main_0.5_inter_0_main_inter <- mapply(FUN = simulation_fn,
                                                              gene_data_args = gene_data_args,
                                                              MoreArgs = list(p = p,
                                                                              tran_fun = null_tran,
                                                                              combine = combine,
