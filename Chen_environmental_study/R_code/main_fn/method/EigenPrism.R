...
EigenPrism_m <- function(y,X,invsqrtSig=NULL,alpha=0.2,target='beta2',zero.ind=c(),diagnostics=F){
  ...
  # Take singular value decomposition and rescale singular values
  svd = try(svd(X%*%t(X)),TRUE)
  if (inherits(svd, "try-error")) {return(rep(999,3))}
    ...
    # Solve optimization problem and extract variables
    opt = try(cps(prob,ctrl(trace=F)),TRUE)
    #opt = cps(prob,ctrl(trace=F))

    if (inherits(opt, "try-error")) {return(rep(999,3))}
    else{
      ...
      svd_x = svd(X)
      subset = min(n,p)
      est = sum(w[1:subset]*(t(svd_x$u)%*%y)^2)
      yvar = sum(y^2)/n
