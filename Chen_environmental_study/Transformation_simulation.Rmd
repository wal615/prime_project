...
- Given a fixed main and interaction effect, 20 values of $y_i$ are randomly generated to calculated the estimated variances
...
- GCTA_mian (in pink) represents the estimated main effect's variance by GCTA method 
...
- However, for the intercation effect, the proposed method seems to have a bais in estimating the interaction variance. 

## Quantile tranformation
- In this case, normal quantile are used the replace the orignal value for each variable of PCB data

* the Normal quantile Procedure is 
    1. Calculate the empirical precentails for each column of PCB data 
    1. Using the precentails to find the corresponding quantile value based on a Standard Normal distrbution.
