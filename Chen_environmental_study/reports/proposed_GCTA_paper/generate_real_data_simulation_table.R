...
file_list <- file_list_0[grep(x = file_list_0, pattern = "nhanceMiceImpute")]
imputed_hemoglobin_total <- lapply(file_list, function (x) {readRDS(x) %>% rbindlist(.)}) %>% rbindlist(.)
...
col_order <- c("data_name", "est_model", "tran_fn_y", "tran_fn_x",
               "y_var","GCTA_total", "GCTA_precentage", "proposed_GCTA_total", "proposed_GCTA_precentage")
imputed_hemoglobin_total_mean_summary <- imputed_hemoglobin_total_mean_summary[, ..col_order]
imputed_hemoglobin_total_mean_summary <- imputed_hemoglobin_total_mean_summary %>% setorder(., data_name, tran_fn_y, tran_fn_x) %>% data.frame(.)
# names(imputed_hemoglobin_total_mean_summary)[1] <- "imputation"
# imputed_hemoglobin_total_mean_summary$imputation <- ifelse(imputed_hemoglobin_total_mean_summary$imputation =="0", "method 1","method 2")

## round to digital 3
col_names <- colnames(imputed_hemoglobin_total_mean_summary)[-(1:5)]
imputed_hemoglobin_total_mean_summary[, -(1:5)] <- imputed_hemoglobin_total_mean_summary[, -(1:5)] %>% round(.,2)
colnames(imputed_hemoglobin_total_mean_summary) <- gsub("_", " ", colnames(imputed_hemoglobin_total_mean_summary))
write.csv(imputed_hemoglobin_total_mean_summary, file = "./reports/proposed_GCTA_paper/nhance/hemolobin/prime_imputed_hemoglobin_total_table_10_25.csv", row.names = FALSE)
