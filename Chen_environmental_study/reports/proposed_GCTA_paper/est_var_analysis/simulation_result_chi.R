...
## GCTA
## full data
mean_table <- sub_result[, .(mean_main_full = mean(GCTA_main), 
                             mean_inter_full = mean(GCTA_inter))]
var_table <- sub_result[, .(var_main_full = var(GCTA_main), 
                            var_inter_full = var(GCTA_inter))]
summary_list <- append(summary_list, list(full_data_mean = mean_table, 
                                          full_data_var = var_table))
## sub-sampling_mean
sub <- sub_result[, .(sub_main_mean = mean(sub_GCTA_main, na.rm = T),
                      sub_inter_mean = mean(sub_GCTA_inter, na.rm = T),
                      sub_main_var = var(sub_GCTA_main, na.rm = T),
...
mean_table <- sub_result[, .(mean_total_full = mean(GCTA_total), 
                             mean_inter_full = mean(GCTA_inter))]
var_table <- sub_result[, .(var_total_full = var(GCTA_total), 
                            var_inter_full = var(GCTA_total))]
summary_list <- append(summary_list, list(mean_table, var_table))
## sub-sampling_mean
sub <- sub_result[, .(sub_total_mean = mean(sub_GCTA_total, na.rm = TRUE),
                      sub_inter_mean = mean(sub_GCTA_inter, na.rm = TRUE),
                      sub_total_var = var(sub_GCTA_total, na.rm = TRUE),
