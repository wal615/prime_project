...
# correlation control groups with p = 100 d = 1 
#############################################################################################################################
...
sub_result[,var_main_effect := (7/9)^3] # the targe is the median of chi with df = 1
...
summary_final_cor_8_d_1_100[, d:=d]

# median control groups d = 1 
#############################################################################################################################

result_path <- "decor_None_sparse_None_chi_structure_I_main_0.5_inter_0_n_50_75_100_150_200_p_2_rho_e_0.5_decor_FALSE_subd_1_iter_1000_nsub_0_median_null_est_main_c_betam_8_c_betai_2_Var_null"
file_list_all <- list.files(paste0("./", result_path, "/")) %>% paste0(paste0("./", result_path, "/"),.)
file_list <- file_list_all
sub_result <- lapply(file_list, function (x) {read.csv(x, header = TRUE, stringsAsFactors = FALSE)}) %>% rbindlist(., fill = TRUE)
sub_result[,var_main_effect := NULL] 
sub_result[,var_main_effect := 0.5] # the targe correlation is 0.5
...
result_path <- "decor_None_sparse_None_chi_structure_I_main_0.5_inter_0_n_50_75_100_150_200_p_2_rho_e_0.5_decor_FALSE_subd_25_38_50_75_100_iter_1000_nsub_5000_median_null_est_main_c_betam_8_c_betai_2_Var_null/"
file_list_all <- list.files(paste0("./", result_path, "/")) %>% paste0(paste0("./", result_path, "/"),.)
file_list <- file_list_all
sub_result <- lapply(file_list, function (x) {read.csv(x, header = TRUE, stringsAsFactors = FALSE)}) %>% rbindlist(., fill = TRUE)
sub_result[,var_main_effect := NULL] 
