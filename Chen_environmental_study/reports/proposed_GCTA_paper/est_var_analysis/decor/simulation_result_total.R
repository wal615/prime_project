...
# least_square
summary_result_least <- sub_result[, .(est_mean = mean(least_square_total, na.rm = T),
                                       var_total_effect = var_total_effect[1]), by = i][, .(MES = mean((est_mean - mean(var_total_effect))^2),
...
sub_summary_result_least_i <- sub_result[, .(sub_est_mean = mean(sub_least_square_total, na.rm = TRUE),
                                             var_jack = jack_var(sub_least_square_total, pro = pro),
                                             sub_z_length = sub_CI_lenght(sub_least_square_total, pro = pro,z_p = z_p),
                                             sub_z_coverage = sub_coverage_rate_z(sub_least_square_total, mean(var_total_effect), upper = upper, lower = lower, pro = pro)),
                                         by = .(n,rho_e, p,i,pro)]
...
summary_result_GCTA <- sub_result[, .(est_mean = mean(GCTA_total, na.rm = T),
                                      var_total_effect = var_total_effect[1]), by = i][, .(MES = mean((est_mean - var_total_effect)^2, na.rm = T),
                                                                                         est_var = var(est_mean),
                                                                                         est_mean = mean(est_mean))]
...
write.csv(table, file = "~/dev/projects/Chen_environmental_study/reports/proposed_GCTA_paper/est_var_analysis/decor/total_result.csv", row.names = F)
table <- read.csv("~/dev/projects/Chen_environmental_study/reports/proposed_GCTA_paper/est_var_analysis/decor/total_result.csv",stringsAsFactors = F) %>% data.table(.) 
table <- table[x_dist != "PCB"]
table[,X:=NULL]
table_final <- rbindlist(list(table,summary_tmp)) 
table_final %>% setorder(., target, x_dist,decor,method)

table_final <- table[,c("var_total_effect","x_dist","decor","method","est_mean","est_var","MES",
         "sub_est_mean","var_jack","sub_z_length","sub_z_coverage")] %>% setorder(., x_dist,decor,method)
names <- c("var_total_effect","est_mean","est_var", "MES", "sub_est_mean", "var_jack","sub_z_length","sub_z_coverage")
