...
combine_csv_fn <- function(dir, folder, rm_original = FALSE){
  path <- paste0(dir,folder,"/")
  file_list <- list.files(path = path, full.names = TRUE)
  sub_result <- lapply(file_list, function (x) {read.csv(x, header = TRUE, stringsAsFactors = FALSE)}) %>% rbindlist(., fill = TRUE)
  saveRDS(object = sub_result, file = paste0(path,folder))
  if (rm_original == TRUE){
    junk <- list.files(path = path, pattern = "_iteration_", full.names = TRUE)
    file.remove(junk)
  }
  cat("write ", paste0(path,folder), "done", "\n")
}
