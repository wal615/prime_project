...
# 
# args <- expand.grid(data_path = wqs, data_name = "wqs_PCBs", pro = pro, reduce_coef = reduce_coef, stringsAsFactors = FALSE)
# 
# gene_data_args <- args[,-4]
# gene_data_args <- gene_data_args %>% split(x = ., f = seq(nrow(gene_data_args))) # generate a list from each row of a dataframe
# 
# dim_red_args <- args[,4, drop = FALSE]
# dim_red_args <- dim_red_args %>% split(x = ., f = seq(nrow(dim_red_args)))
# 
# uncorr_args <- list(p = 34)
# # inter_stadardized
# result_list_real_wqs_dim_reduction <- mapply(FUN = fit_real_data_fn,
...
#                                SIMPLIFY = FALSE)
# 
# saveRDS(result_list_real_wqs_dim_reduction, file = "./result/simulation_proposed_GCTA_paper/result_list_real_wqs_dim_reduction")
