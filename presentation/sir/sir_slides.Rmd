...
### Regression Analysis
- Study the relationship of a response y and its covariates x
- Use the information of x to explain y (Forward Rregression)
...
- One solustion is reducing the demension of x  
...
#### What will be the best direction to visualize data? 
```{r, echo = FALSE, out.width="32%"}
myimages<-list.files("./pic/", pattern = "toy", full.names = TRUE)
...
- Use the information of y to explain x (Inverse Regression) 
...
\[
  y = f(\beta_1X, \dots, \beta_kX, \epsilon) \Leftrightarrow (Y \Perp X) |\beta_1X, \dots, \beta_kX  
\]

#### Condition 3.1 
Linear Condition  
For any $b$ in $\mathcal{R}^p$, 
\[E(bX|\beta_1X = \beta_1x, \dots, \beta_kX = \beta_kx) = c_0 + c_1\beta_1x, \dots, c_k\beta_kx\]
...
### Slice Inverse Regression Method
   
![](./pic/sir_method.png){width=500px}



### Sliced Inverse Regression Method with detials
...
- $\beta$ and $\hat{\beta}$ are standardized 
...
\begin{itemize}
    ...
    \item $\sigma = (0.5, ~1) $
    \item The number of slice $H = (5, 10, 20)$ 
    \item Two components $\beta_1 = (1,0,0,\dots,0), ~ \beta_2 = (0,1,0,\dots,0)$
\end{itemize}
\end{exampleblock}
...
#### Criterion of the subspace $\mathcal{B}$
\[
...
### Graphics 
![Response surface of simulation 3](./pic/surface.png){height=100px}

#### How to visualize the data?
\href{http://www.stat.ucla.edu/~kcli/sir-plot.pdf}{Directions found by SIR} 
