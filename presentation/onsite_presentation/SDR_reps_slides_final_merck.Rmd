...
- Span: Given $\mathbf{A} = (\vec{\mathbf{a}}_1, \dots, \vec{\mathbf{a}}_k), ~~\vec{\mathbf{a}}_k \in \mathbb{R}^n$, a k-dimension subspace $V$ spanned by $\mathbf{A}$ is all the linear combinations of $(\vec{\mathbf{a}}_1, \dots, \vec{\mathbf{a}}_k)$
\[
...
1. The covariance matrix of $E(X|Y) - E(X)$ is degenerate at any direction that orthogonal to $b_k\Sigma_{X},k = 1, \dots, K$
1. Condidate Matrix:  $M_{SIR} = Var(E(X|Y))$
1. $S_{SIR} := Span(\Sigma_{X}^{-1}M) \subseteq S_{Y|X}$
1. $\Sigma_{X}^{-1}Mb = \lambda b \Rightarrow Mb = \lambda\Sigma_{X}b$
...
### How to estimate the $E(X|Y)$?

1. Sort the data based on the response $Y_1 \dots, Y_n$     
1. Split data into H slices based on the sorted responses$Y^{(1)},\dots,Y^{(n)}$  
1. Within each slice, replace the responses with slice lable $h \in {1, \dots, H}$  
1. Within the slice h, calculate the average of X, $\tilde{X}_h = \hat{E}(X|Y = \tilde{Y}_h)$
![](./pic/slice method.png){width=400px}  


### Issue with Binary response

Binary response only has two levels, e.g. $0,1$. So that only two slices are available

#### 2 slices affects the performances of SDR method
